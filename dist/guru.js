...
cEvent.prototype.listen = function listen (event, callback) {
    document.body.addEventListener(event, callback);
};

cEvent.prototype.dispatch = function dispatch (event, data) {
...
var Signal = new cEvent();

riot$1.tag2('item', '<li class="item" onclick="{toggleHighlightElement}"> {buildIdentifier(opts.element.localName, opts.element.className, opts.element.id, opts.element.text)} </li>', 'item .item,[data-is="item"] .item{ display: inline-block; border: 1px solid gray; margin: 2px; padding: 0 2px; line-height: 1.4; font-family: sans-serif; color: #fff; }', '', function(opts) {
    this.elementHighlighted = false;

    this.buildIdentifier = function(localName, className, id, text) {
    ...
    this.toggleHighlightElement = function(event) {
        this.opts.element.style.border = (!this.elementHighlighted) ? '4px solid red': '';
        this.elementHighlighted = !this.elementHighlighted;
    }.bind(this);
});

riot$1.tag2('debugger-line', '<div class="debugger-line-wrapper" if="{foundElements.length > 0}"> <div class="debugger-line" onclick="{showItemsToggle}"> Found <span class="{foundElements.length == 0 ? ⁗good⁗: ⁗bad⁗}">{foundElements.length}</span> {opts.hinter.description.toLowerCase()} </div> <ul if="{showItems}"> <item each="{foundElements}" element="{this}"></item> </ul> </div>', 'debugger-line h1,[data-is="debugger-line"] h1{ margin: 0; } debugger-line .good,[data-is="debugger-line"] .good{ color: green; } debugger-line .bad,[data-is="debugger-line"] .bad{ color: red; } debugger-line .debugger-line-wrapper > div,[data-is="debugger-line"] .debugger-line-wrapper > div{ font-size: 14px; } debugger-line ul,[data-is="debugger-line"] ul{ font-size: 13px; margin: 0; padding: 0; } debugger-line .debugger-line,[data-is="debugger-line"] .debugger-line{ cursor: pointer; }', '', function(opts) {
    ...
    this.showItemsToggle = function(event) {
        this.showItems = !this.showItems;
    }.bind(this);
});

var dexie = createCommonjsModule(function (module, exports) {
(function (global, factory) {
...
var _global = typeof self !== 'undefined' ? self : typeof window !== 'undefined' ? window : commonjsGlobal;
        ...
        _unlock: function () {
            ...
            if (--this._reculock === 0) {
                ...
                while (this._blockedFuncs.length > 0 && !this._locked()) {
                    var fnAndPSD = this$1._blockedFuncs.shift();
                    try {
...
riot$1.tag2('scanner', '<div class="scan_results"> <table> <tr each="{pageScanResults}"> <td>{url}</td> <td>{found}</td> <td>{status}</td> <td>{visited}</td> </tr> </table> </div>', 'scanner .scan_results,[data-is="scanner"] .scan_results{ position: absolute; top: 0; right: 0; width: 400px; height: 400px; background-color: gray; padding: 4px; overflow-y: scroll; }', '', function(opts) {
    var pageScanResults = [];
    var self = this;
...
riot$1.tag2('debugger', '<scanner if="{scanner_active}"></scanner> <div class="results"> <main if="{validator_open}"> <debugger-line each="{opts.hinters}" hinter="{this}"></debugger-line> </main> <main if="{scanner_open}"> <toggle-button onclick="{toggleScan}" active="{scanner_active}">{parent.scanner_active ? \'stop\' : \'start\'} scan</toggle-button> </main> <header> <h1>guru</h1> <div class="options"> <toggle-button onclick="{toggleScanner}" active="{scanner_open}">scanner</toggle-button> <toggle-button onclick="{toggleValidator}" active="{validator_open}">validator</toggle-button> </div> </header> </div>', 'debugger header,[data-is="debugger"] header{ display: flex; flex-wrap: nowrap; justify-content: space-between; align-items: center; } debugger main,[data-is="debugger"] main{ margin-bottom: 15px; } debugger h1,[data-is="debugger"] h1{ margin: 0; color: #b0d6f4; } debugger .results,[data-is="debugger"] .results{ width: 300px; max-height: 100vh; position: fixed; padding: 10px 15px; bottom: 0px; right: 50px; background-color: #000; z-index: 1000; font-family: sans-serif; color: #fff; transition: height 0.7s; } debugger .options,[data-is="debugger"] .options{ display: flex; flex-direction: row; } debugger .options > div,[data-is="debugger"] .options > div{ cursor: pointer; } debugger .options > div:first-child,[data-is="debugger"] .options > div:first-child{ margin-right: 10px; }', '', function(opts) {
    ...
    this.toggleValidator = function(event) {
        this.validator_open = !this.validator_open;
    }.bind(this);

    this.toggleScan = function(event) {
...
var queryString$1 = createCommonjsModule(function (module) {
/*!
	query-string
...
(function () {
	...
	if (typeof undefined === 'function' && undefined.amd) {
		undefined(function() { return queryString; });
